...
NavPath NavMesh::computePathSmooth(const Vec3& start, const Vec3& end) const
{
    NavPath navpath;
    static const int MAX_POLYS = 256;
    float straightPath[MAX_POLYS*3];
    ...
    if (npolys)
    {
        ...
        m_navQuery->closestPointOnPoly(startRef, &posStart.x, iterPos, 0);
        m_navQuery->closestPointOnPoly(polys[npolys-1], &posEnd.x, targetPos, 0);
        
        static const float STEP_SIZE = 0.5f;
        static const float SLOP = 0.01f;
        
        int nsmoothPath = 0;
        navpath.mPoints.push_back(Vec3(iterPos[0], iterPos[1], iterPos[2]));
