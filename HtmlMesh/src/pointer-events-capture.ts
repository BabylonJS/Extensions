...
export const requestCapture = (
    requestId: string,
    captureCallback: CaptureReleaseCallback,
    releaseCallback: CaptureReleaseCallback
) => {
    debugLog(
        `In pointerEventsCapture.requestCapture - Pointer events capture requested for ${requestId}`
    );

    // If there is a release for this request, then ignore the request
    if (removeUnmatchedRequest(requestId)) {
        debugLog(
            `In pointerEventsCapture.requestCapture - Capture request matched previous release request ${requestId}.  Cancelling capture request`
        );
        return;
...
export const requestRelease = (requestId: string | null) => {
    debugLog(
        `In pointerEventsCapture.requestRelease - Pointer events release requested for ${requestId}`
    );
    ...
    if (!requestId || requestId === currentOwner) {
        ...
        debugLog(
            `In pointerEventsCapture.requestRelease - Received release request ${requestId} but no matching capture request was received`
        );
...
const enqueueCaptureRequest = (
    requestId: string,
    capture: CaptureReleaseCallback,
    release: CaptureReleaseCallback
) => {
    debugLog(
        `In pointerEventsCapture.enqueueCaptureRequest - Enqueueing capture request for  ${requestId}`
    );
...
const cancelRequest = (requestId: string | null) => {
    ...
    captureRequestQueue = captureRequestQueue.filter((id) => {
        if (id !== requestId) {
            ...
            removed = true;
            debugLog(
                `In pointerEventsCapture.cancelRequest - Canceling pointer events capture request ${requestId}`
            );
            return false;
        }
    });
...
const transferPointerEventsOwnership = () => {
    ...
    debugLog(
        `In pointerEventsCapture.transferPointerEventsOwnership - Transferrring pointer events from ${currentOwner} to ${newOwnerId}`
    );
...
const doRelease = () => {
    debugLog(
        `In pointerEventsCapture.doRelease - Releasing pointer events from ${currentOwner}`
    );
    if (currentOwner) {
...
const doCapture = (newOwnerId: string) => {
    ...
    debugLog(
        `In pointerEventsCapture.doCapture - Pointer events now captured by ${newOwnerId}`
    );
};
...
const debugLog = (message: string) => {
    ...
    if (
        ...
        window["pointer-events-capture-debug"]
    ) {
        console.log(
