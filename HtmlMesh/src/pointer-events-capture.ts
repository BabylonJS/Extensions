...
const enqueueCaptureRequest = (requestId: string, capture: CaptureReleaseCallback, 
                         release: CaptureReleaseCallback) => {
    ...
    if (!captureRequestQueue.includes(requestId)) {
        ...
        pendingRequestCallbacks.set(requestId,
            { capture, release });
    }
};
...
const nextCaptureRequest = () => {
    return captureRequestQueue.length > 0 ?
...
const debugLog = (message: string) => {
    ...
    if (typeof window === 'undefined' || window["pointer-events-capture-debug"]) {
        console.log(`${performance.now()} - game.scene.pointerEvents - ${message} `, 
                    `currentOwner: ${currentOwner}`, `queue: ${captureRequestQueue}`, 
